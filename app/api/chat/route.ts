...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Anonymous (guest) users are limited to Quick mode: never trust an
    // 'adaptive' cookie for guests, otherwise the auth gate below would block
    // every message even when the UI shows Quick.
    if (isGuest) {
      searchMode = 'quick'
    }
