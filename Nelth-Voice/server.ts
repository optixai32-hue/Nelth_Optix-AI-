...
const POPULAR_VOICES: VoiceInfo[] = [
  {
    ...
    isUniversal: true,
    description:
      'Modèle neural universel capable de parler couramment français, anglais, espagnol, arabe, allemand, japonais, etc. avec une seule voix.'
  },
  {
  ...
  },
  {
    ...
    gender: 'Male',
    locale: 'fr-FR',
    displayName: 'Henri (Français - France)'
  },
  {
    name: 'fr-FR-EloiseNeural',
    gender: 'Female',
    locale: 'fr-FR',
    displayName: 'Éloïse (Français - France)'
  },
  {
    name: 'fr-FR-VivienneMultilingualNeural',
    gender: 'Female',
    locale: 'fr-FR',
    displayName: 'Vivienne (Français Multilingue)'
  },
  {
    name: 'fr-FR-RemyMultilingualNeural',
    gender: 'Male',
    locale: 'fr-FR',
    displayName: 'Rémy (Français Multilingue)'
  },
  {
  ...
  },
  {
    ...
    gender: 'Female',
    locale: 'es-ES',
    displayName: 'Elvira (Español - España)'
  },
  {
    name: 'de-DE-KatjaNeural',
    gender: 'Female',
    locale: 'de-DE',
    displayName: 'Katja (Deutsch - Deutschland)'
  },
  {
    name: 'it-IT-ElsaNeural',
    gender: 'Female',
    locale: 'it-IT',
    displayName: 'Elsa (Italiano - Italia)'
  }
]
...
async function startServer() {
  ...
  app.post('/api/speech-turn', async (req, res) => {
    try {
      ...
      const { streamGenerator, modelUsed } = await acquireLLMStream(
        nvidiaMessages,
      ...
      res
        .status(500)
        .json({ error: err?.message || 'Erreur de génération de réponse AI' })
    }
  })
  ...
  function parseApiErrorMessage(err: any): string {
    ...
    try {
      const obj =
        typeof err === 'object' && err !== null ? err : JSON.parse(str)
    ...
    if (
      str.includes('NVIDIA_API_KEY') ||
      str.includes('MISSING_NVIDIA_API_KEY')
    ) {
  ...
  async function* streamNemotronLightning(
    ...
    if (totalYielded === 0 && !abortSignal.aborted) {
      throw new Error(
        `Le modèle ${TARGET_LLM_MODEL} a terminé le flux sans texte émis.`
      )
    }
  }
  ...
  async function* streamGeminiFlash(
    ...
    if (!ai) {
      throw new Error(
        'Ni NVIDIA_API_KEY ni GEMINI_API_KEY ne sont configurées sur le serveur.'
      )
    }
    ...
    console.log(
      `[Gemini Flash LLM] Streaming response from gemini-2.5-flash...`
    )
  ...
  async function acquireLLMStream(
    ...
    if (apiKey) {
      try {
        ...
        if (!first.done || first.value) {
          ...
          console.log(
            `[NVIDIA] Stream successfully opened for ${TARGET_LLM_MODEL}`
          )
          return {
        ...
        console.warn(
          `[NVIDIA] Call failed or key issue: ${nvidiaErr?.message || nvidiaErr}. Falling back to Gemini 2.5 Flash...`
        )
      }
    } else {
      console.log(
        `[LLM] NVIDIA_API_KEY non fournie. Bascule automatique sur Gemini 2.5 Flash...`
      )
    }
    ...
    if (firstGemini.done && !firstGemini.value) {
      throw new Error(
        "Le modèle de secours Gemini 2.5 Flash n'a renvoyé aucun contenu."
      )
    }
    ...
    ws.on('message', async rawMessage => {
      try {
        ...
        if (payload.type === 'tts_request') {
          ...
          try {
            ...
            ws.send(
              JSON.stringify({
                type: 'error',
                error: ttsErr?.message || 'TTS error'
              })
            )
          }
          return
        ...
        if (payload.type === 'user_speech') {
          ...
          if (!userText) {
            ws.send(
              JSON.stringify({ type: 'error', error: 'Empty speech text' })
            )
            return
          }
          ...
          for await (const textDelta of streamGenerator) {
            ...
            const { chunks, remainder } = extractSpokenChunks(
              textBuffer,
              isFirstChunk
            )
            if (chunks.length > 0) {
