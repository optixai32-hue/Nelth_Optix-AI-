...
export const modelSelectorTriggerVariants = cva(
  "group focus-visible:ring-ring/50 flex w-fit items-center justify-between gap-2 overflow-hidden rounded-xl text-sm font-medium whitespace-nowrap transition-all duration-200 outline-none focus-visible:ring-2 disabled:cursor-not-allowed disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 data-[state=open]:bg-accent/60",
  {
    variants: {
      variant: {
        outline:
          'border-input bg-background/60 shadow-sm backdrop-blur hover:-translate-y-px hover:border-primary/40 hover:shadow-md',
        ghost: 'hover:bg-accent/80 hover:text-accent-foreground',
        muted:
          'border border-transparent bg-secondary/60 text-secondary-foreground backdrop-blur hover:border-border/60 hover:bg-secondary/80'
      },
      size: {
...
function ModelSelectorTrigger({
  ...
  return (
    ...
    >
      ...
      <ChevronDownIcon className="size-4 shrink-0 opacity-50 transition-transform duration-200 group-data-[state=open]:rotate-180 group-hover:scale-110" />
    </PopoverTrigger>
  )
}
...
function ModelIcon({
  ...
  return (
    <span
      className={cn(
        'flex size-5 shrink-0 items-center justify-center rounded-md bg-muted text-muted-foreground shadow-sm [&_svg]:size-3.5',
        className
      )}
    >
      {children}
...
function ModelSelectorContent({
  ...
  return (
    <PopoverContent
      ...
      className={cn(
        'bg-popover/70 w-72 min-w-(--radix-popover-trigger-width) overflow-hidden rounded-2xl border border-border/50 p-0 shadow-xl shadow-black/5 ring-1 ring-black/5 backdrop-blur-xl transition-[opacity,transform] duration-200 ease-out data-[state=closed]:scale-95 data-[state=closed]:opacity-0 data-[state=open]:scale-100 data-[state=open]:opacity-100 dark:ring-white/10',
        className
      )}
      {...props}
    >
...
function ModelSelectorItem({
  ...
  return (
    <CommandItem
      ...
      className={cn(
        "group/item relative flex items-start gap-2.5 rounded-xl py-2 ps-3 pe-9 transition-all duration-150 hover:bg-accent/60 hover:shadow-sm [&_svg:not([class*='size-'])]:size-3.5",
    ...
    >
      {children ?? (
        <>
          {model.icon && (
            <ModelIcon className="mt-[5px]">{model.icon}</ModelIcon>
          )}
      ...
      {isSelected && (
        <span className="absolute end-3 top-2.5 flex size-4 items-center justify-center text-primary">
          <CheckIcon className="size-4 transition-transform duration-200 group-data-[state=open]:scale-100" />
        </span>
      )}
    </CommandItem>
  )
...
function ModelSelectorEffort({
  ...
  return (
    <div
      ...
      className={cn(
        'flex cursor-default items-center justify-between gap-3 border-t border-border/50 bg-muted/30 px-3 py-2.5',
        className
      )}
      onKeyDown={e => {
        onKeyDown?.(e)
    ...
    >
      ...
      <RadioGroupPrimitive.Root
        ...
        className="flex items-center gap-1 rounded-full bg-muted/80 p-0.5"
      >
        {efforts.map(option => (
          <RadioGroupPrimitive.Item
            ...
            value={option.id}
            className={cn(
              'rounded-full px-2.5 py-1 text-xs transition-all duration-200 outline-none focus-visible:ring-2 focus-visible:ring-ring/50',
              'text-muted-foreground hover:text-foreground',
              'data-[state=checked]:bg-background data-[state=checked]:font-semibold data-[state=checked]:text-foreground data-[state=checked]:shadow-sm'
            )}
          >
            {option.name}
          </RadioGroupPrimitive.Item>
