...
function titleCaseLabel(label: string): string {
  ...
  return label.split('-').filter(Boolean).map(formatSiteToken).join(' ')
}
...
type CitationCarouselContentProps = React.ComponentProps<typeof CarouselContent>
...
function CitationItem({
  ...
  const safeHref = resolvedHref
    ? parseCitationUrlOrNull(resolvedHref)?.href
    : ''
...
function CitationSource({ className, ...props }: CitationSourceProps) {
  return (
    <div
