...
export function Chat({
  ...
  const isCurrentAdaptiveModeAuthBlocked = useCallback(
    () =>
      // Guests are handled by the SearchModeSelector (it prompts to sign in
      // when Adaptive is clicked). The send/regenerate guard must not read
      // the cookie here, otherwise a stale 'adaptive' cookie would pop the
      // auth modal even while the guest is using Quick mode.
      isGuest
        ? false
        : isAdaptiveModeAuthBlocked({
  ...
  const sections = useMemo<ChatSection[]>(() => {
    ...
    for (const message of messages) {
      // Never mix messages from a different conversation into this chat.
      const convId = (message.metadata as any)?.conversationId
      if (convId && convId !== chatId) continue

      // Deduplicate by message.id — the database is the source of truth, but
      // optimistic client updates can surface the same message twice during
      // stream finalization. Keep the first occurrence to preserve order.
  ...
  // Anonymous (guest) users are limited to Quick mode. Force the searchMode
  // cookie to 'quick' on mount (and whenever guest state changes) so the
  // send/regenerate guards that read the cookie never mistake Quick for
  // Adaptive and pop the auth modal.
  useEffect(() => {
    if (!isGuest) return
    if (getCookie('searchMode') !== 'quick') {
      setCookie('searchMode', 'quick')
    }
  }, [isGuest])

  // Anonymous (guest) conversations are not persisted: warn on refresh/close
  // so the user knows the discussion will be lost. The browser's native
  // confirm handles the choice (yes = leave & lose, no = stay).
  useEffect(() => {
    if (!isGuest) return
