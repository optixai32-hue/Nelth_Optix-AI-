 ...
 */
import { CONVERSATION_CONTINUITY_POLICY } from '@/lib/conversation/continuity-policy'
export { CONVERSATION_CONTINUITY_POLICY }
...
/**
 * Full CORE DIRECTIVE — the single authoritative behavioral contract for BOTH
 * models, placed at the VERY TOP of the system instructions (primacy bias).
 * Weak models follow terse checklists poorly and answered generically, so
 * both models receive the complete contract: identity, conversation
 * continuity, tool usage, skill enforcement, no-visualization, document
...
export async function createResearcher({
  ...
  try {
    ...
    // CORE DIRECTIVE — the FULL contract (FULL_CORE_DIRECTIVE) is placed at
    // the VERY TOP (primacy bias) so every model reads and honours it before
    // any other content. Formerly the weak model got a compact 5-rule header
    // to save prefill tokens — but weak models follow terse checklists poorly
...
- FAST DIRECT ANSWERS when confident + AUTOMATIC WEB RESEARCH when necessary.
- CRITICAL — ACTION OVER WORDS: When you decide to search the web, execute the search tool call SILENTLY. NEVER output announcement text such as "Je fais une recherche web...", "Recherche web en cours...", "Laissez-moi chercher...", "I am searching the web...", or "Let me search...". DO NOT write any introductory or conversational text before or instead of calling the tool. Call the search tool immediately and natively. Output text ONLY AFTER receiving the search results.
- SEARCH THE WEB WHEN:
...
- VALID JAVASCRIPT ONLY (the preview executes it): French apostrophes MUST NOT break strings — write "l'utilisateur", "d'aujourd'hui" with DOUBLE quotes (or escape them), NEVER a raw ' inside single-quoted strings; same inside HTML onclick="..." attributes. Close every (, [, {; no stray backticks; never write a literal "</script>" inside a JS string.
