...
export const fetchTool = tool({
  ...
  async *execute({ url, type = 'regular' }) {
    ...
    if (type === 'regular') {
      ...
      // Use API-based extraction (Jina Reader). Fall back to a direct regular
      // fetch when no Jina key is configured, since the previous Tavily
      // Extract dependency has been removed.
      const useJina = process.env.JINA_API_KEY
      if (useJina) {
        results = await fetchJinaReaderData(url)
      } else {
        console.warn(
          'Fetch "api" type requested but JINA_API_KEY is not set; falling back to regular fetch.'
        )
        results = await fetchRegularData(url)
      }
    }
